Shader "Custom/WobbleShader"
{
    ...
    SubShader
    {
        ...
        void vert(inout appdata_full data){
            ...
            modifiePos.y += sin(data.vertex.z * _Frq + _Time.y * _AnimSpeed) * _Amp;

            float3 posPlusTangent = data.vertex + data.tangent * 0.01;
            posPlusTangent.y += sin(posPlusTangent.z *  _Frq + _Time.y * _AnimSpeed) * _Amp;

            float3 bitTangent = cross(data.normal, data.tangent);
            float3 posPlusBitTangent = data.vertex + bitTangent * 0.01;
            posPlusBitTangent.y  += sin(posPlusBitTangent.z *  _Frq + _Time.y * _AnimSpeed) * _Amp;
