...
namespace Doozy.Editor.Soundy
{
    ...
    public class SoundGroupDataEditor : BaseEditor
    {
        ...
        {
            SerializedProperty priority = GetProperty(PropertyName.Priority);
            SerializedProperty min = GetProperty(PropertyName.MinValue, priority);
            SerializedProperty max = GetProperty(PropertyName.MaxValue, priority);

            DGUI.Line.Draw(false, ComponentColorName,
                           () =>
                           {
                               GUILayout.Space(DGUI.Properties.Space(2));
                               DGUI.Label.Draw(UILabels.PriorityDb, Size.S, ComponentColorName, DGUI.Properties.SingleLineHeight); //priority label
                               DGUI.Property.Draw(GetProperty(PropertyName.Priority), DGUI.Properties.SingleLineHeight);//priority property
                               GUILayout.Space(DGUI.Properties.Space());
                               if (DGUI.Button.IconButton.Reset(DGUI.Properties.SingleLineHeight, ComponentColorName))
                               {
                                   Undo.RecordObject(this, "Reset Priority");
                                   Target.Priority = SoundGroupData.DEFAULT_PRIORITY;
                                   m_needsSave = true;
                               }
